...
function RulesPage() {
  ...
  // 构造静态文件URL
  const getStaticUrl = (fileName) => `http://localhost:8000/static/reports/${encodeURIComponent(fileName)}`
  ...
  const fetchFiles = async () => {
    ...
    setError('')
    try {
      const res = await apiClient.get('/api/reports/list')
  ...
  const handleDelete = async (fileName) => {
    setDeleting(true)
    try {
      await apiClient.delete('/api/reports/delete', { params: { file_name: fileName } })
  ...
  const handleBatchDelete = async () => {
    setDeleting(true)
    try {
      await Promise.all(selectedRowKeys.map(fn => apiClient.delete('/api/reports/delete', { params: { file_name: fn } })))
  ...
  const handleView = async (fileName) => {
    ...
    try {
      const res = await apiClient.get('/api/reports/view', { params: { file_name: fileName } })
  ...
  return (
    <div style={{ padding: 32 }}>
      ...
      <Space style={{ marginBottom: 16 }}>
        ...
        <Popconfirm title={`确定删除选中的${selectedRowKeys.length}个规则文件吗？`} onConfirm={handleBatchDelete} okText="删除" cancelText="取消" disabled={selectedRowKeys.length === 0}>
